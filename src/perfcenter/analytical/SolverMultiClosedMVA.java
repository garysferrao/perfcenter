...
public class SolverMultiClosedMVA extends SolverMulti {
	...
	/**
	 * Intermediate results: System response time [customers]
	 */
	protected double[] interm_sysResponseTime;

	/**
	 * Intermediate results: System throughput [customers]
	 */
	protected double[] interm_sysThroughput;

	/**
	 * Intermediate results: Number of jobs in the system [customers]
	 */
	...
	// TODO1: attenzione, non � in grado di gestire classi con popolazioni nulle
	/**
	 * initializes the Multi class solver with the system parameters.
	 * 
	 * @param n
	 *            array of names of the service centers.
	 * @param t
	 *            array of types (LD or LI) of the service centers.
	 * @param s
	 *            matrix of service time of the service centers.
	 * @param v
	 *            matrix of visits to the service centers.
	 * @param pop
	 *            array of populations of each class
	 * @return true if the operation is completed with success
	 */
	public boolean input(String[] n, int[] t, double[][][] s, double[][] v, int[] pop, int[] noserv) {
		int maxPop = 0;
		...
		System.arraycopy(noserv, 0, servers, 0, stations);
		...
		for (int i = 0; i < stations; i++) {
			...
			/*
			 * for (int j = 0; j < classes; j++) servTime[i][j] = new double[totPop];
			 */
			if (t[i] == LD)
		...
		// TODO1: perch�? (per ottimizzare consumo di memoria muovendosi
		// intelligentemente nello spazio degli stati)
		...
		for (int i = 0; i < stations; i++) {
			...
			// TODO1: perch� copia s[i][0].length e non s[i][lastClass].length ?
			// OLD
			// System.arraycopy(s[i][lastClass], 0, servTime[i][maxIndex], 0,
			// s[i][0].length);
			// NEW
			// @author Stefano Omini
	...
	/**
	 * A system is said to have sufficient capacity to process a given load <tt>lambda</tt> if no service center is saturated as a result of the
	 * combined loads of all the classes. <br>
	 * Must be implemented to create a multi class model solver. <br>
	...
	/**
	 * Solves the model, using an appropriate technique (LI or LD model). <br>
	 ...
	 * 
	 * @param intermediate
	 *            true to save intermediate results
	 * 
	 */
	...
	private void solveLI() {
		...
		// queue length\\TODO1: memory????
		n = new double[stations][memory];

		// pi used to store marginal probability
		double pi[][][] = new double[stations][][];

		for (int i = 0; i < stations; i++) {
			pi[i] = new double[servers[i]][memory];
			pi[i][0][0] = 1;
		}
		...
		do {
			// TODO1: tolta per controllare se serve, probabilmente � solo per
			// debug
			// printStatus();
					...
					if (status[c] > 0) {
						if (type[m] == LI) {
							if (servers[m] < 2)
								...
								int temp = 0;
								for (int j = 0; j < servers[m] - 1; j++) {
									temp += (servers[m] - j - 1) * pi[m][j][index - popMult[c]];
								}
								residenceTime[m][c] = demand[m][c] * (1.0 + n[m][index - popMult[c]] + temp) / servers[m];
							}
						} else
				...
				if (status[lastClass] > 0) {
					if (type[m] == LI) {
						if (servers[m] < 2)
							...
							int temp = 0;
							for (int j = 0; j < servers[m] - 1; j++) {
								temp += (servers[m] - j - 1) * pi[m][j][index];
							}
							residenceTime[m][lastClass] = demand[m][c] * (1.0 + n[m][index] + temp) / servers[m];
						}
					} else
			...
			// reposition ??? marginal probability calculation

			double servtemp = 0;
			for (int x = 1; x < servers[0]; x++) {
				double temp = 0;
				for (int y = 0; y < lastClass; y++) {
					if (status[y] > 0) {
						temp += (demand[0][y]) * clsThroughput[y] * pi[0][x - 1][index - popMult[y]]; // to be
																										// verified
																										// (index -
																										// popMult[c])
					}
				...
				if (status[lastClass] > 0) {
					temp += (demand[0][lastClass]) * clsThroughput[lastClass] * pi[0][x - 1][index];
				} // to be verified (index - popMult[c])
				pi[0][x][index] = (1 / x) * temp;
				servtemp += (servers[0] - x) * pi[0][x][index];
			}

			double temp = 0;
			for (int y = 0; y < classes; y++) {
				temp += (visits[0][y] * servTime[0][y][0]) * clsThroughput[y];
			}
			pi[0][0][index] = 1 - (1 / servers[0]) * (servtemp + temp);
		...
		for (m = 0; m < stations; m++) {
			for (c = 0; c < classes; c++) {
				...
				utilization[m][c] = throughput[m][c] * servTime[m][c][0] / servers[m]; // Umc=Xmc*Smc
	...
	private void solveLI(boolean intermediate) {
		...
		// TODO1: ci interessano i risultati intermedi anche per i multiclass??
	...
	// /**
	// * See: <br>
	// * G.Balbo, S.C.Bruell, L.Cerchio, D.Chiaberto, L.Molinatti "Mean Value
	// * Analysis of Closed Load Dependent Networks".
	// */
	...
	// } else {
	// for (int s = 0; s < stations; s++) {
	// if (type[s] == SolverMulti.DELAY) {
	// residenceTime[s + 1][c + 1] = servTime[s][c][0];
	// } else if (type[s] == SolverMulti.LI) {
	// /*
	// * ix[c]--; x = linear( NC - 1 , ix , N ) ;
	// * residenceTime[s+1][c+1] = servTime[s][c][0] * ( 1 +
	// * n[s + 1][x]); ix[c]++;
	// */
	// }
	// }
	...
	public String toString() {
		...
		/*
		 * 
		 * for (int n = 0; n <= customers; n++) { buf.append("\n\nPopulation = " + n + " customers\n\n"); buf.append("Queue Length (Q)");
		 * buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) { buf.append("[ ");
		 * buf.append(Printer.print(interm_queueLen[n][m], 6)); buf.append(" ]\n"); }
		 * 
		 * buf.append("\n\n"); buf.append("Residence Time (R)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations;
		 * m++) { buf.append("[ "); buf.append(Printer.print(interm_residenceTime[n][m], 6)); buf.append(" ]\n"); }
		 * 
		 * buf.append("\n\n"); buf.append("Throughput (X)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations;
		 * m++) { buf.append("[ "); buf.append(Printer.print(interm_throughput[n][m], 6)); buf.append(" ]\n"); }
		 * 
		 * buf.append("\n\n"); buf.append("Utilization (U)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations;
		 * m++) { buf.append("[ "); buf.append(Printer.print(interm_utilization[n][m], 6)); buf.append(" ]\n"); } }
		 */
	...
	// if (status[1] % 3 == 0 && status[1] > 0
	// && ((status[0] / status[1]) == 1 / 3)) {
	 ...
	 * buf.append("\nAnalysis with MVA Multiclass"); buf.append("\n"); buf.append("Throughput "); for (int c = 0; c < classes; c++)
	 * buf.append("class " + (c + 1) + " "); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) {
	 * buf.append("Resource " + m + " "); for (int c = 0; c < classes; c++) buf.append(Printer.print(throughput[m][c], 6) + " "); buf.append("\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Queue Length "); for (int c = 0; c < classes; c++) buf.append("class " + (c + 1) + " ");
	 * buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) { buf.append("Resource " + m + " "); for (int c = 0;
	 * c < classes; c++) buf.append(Printer.print(queueLen[m][c], 6) + " "); buf.append("\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Res Time "); for (int c = 0; c < classes; c++) buf.append("class " + (c + 1) + " ");
	 * buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) { buf.append("Resource " + m + " "); for (int c = 0;
	 * c < classes; c++) buf.append(Printer.print(residenceTime[m][c], 6) + " "); buf.append("\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Utilization "); for (int c = 0; c < classes; c++) buf.append("class " + (c + 1) + " ");
	 * buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) { buf.append("Resource " + m + " "); for (int c = 0;
	 * c < classes; c++) buf.append(Printer.print(utilization[m][c], 6) + " "); buf.append("\n"); } return buf.toString(); }
	 */
	 ...
	 * 
	 * buf.append("\n\n"); buf.append("Throughput (X)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) {
	 * buf.append("[ "); for (int c = 0; c < classes; c++) buf.append(Printer.print(throughput[m][c], 6) + " "); buf.append("]\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Queue Length (Q)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++)
	 * { buf.append("[ "); for (int c = 0; c < classes; c++) buf.append(Printer.print(queueLen[m][c], 6) + " "); buf.append("]\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Residence Time (R)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations;
	 * m++) { buf.append("[ "); for (int c = 0; c < classes; c++) buf.append(Printer.print(residenceTime[m][c], 6) + " "); buf.append("]\n"); }
	 * 
	 * buf.append("\n\n"); buf.append("Utilization (U)"); buf.append("\n------------------------------------\n"); for (int m = 0; m < stations; m++) {
	 * buf.append("[ "); for (int c = 0; c < classes; c++) buf.append(Printer.print(utilization[m][c], 6) + " "); buf.append("]\n"); } return
	 * buf.toString(); }
	 */
	...
	// */
	// System.out.println(model.getClsThroughput(0) + " "
	// + model.getClsThroughput(1));
	//
	// }
	// }
