...
/**
 * SolverMulti is an abstract class, which contains the methods initialize and solve multiclass models.
 * 
...
public abstract class SolverMulti {
	 ...
	 */
	public boolean input(String[] n, int[] t, double[][][] s, double[][] v, int[] sn) {
		...
		System.arraycopy(sn, 0, servers, 0, stations);
	...
	/**
	 * Prints all the calculated indexes in the system.out. It is used to debug or test the class.
	 */
	 ...
	 * OLD
	 * 
	 * public String toString() { StringBuffer buf = new StringBuffer(); buf.append(getClass().getName());
	 * buf.append("\n------------------------------------"); buf.append("\nAnalysis with SolverMulti"); buf.append("\n"); for (int i = 0; i <
	 * stations; i++) { buf.append("\n\n\nIndexes of " + name[i]); for (int j = 0; j < classes; j++) { buf.append("\n\nClass " + j);
	 * buf.append("\n- queue length      : " + queueLen[i][j]); buf.append("\n- residence time    : " + residenceTime[i][j]);
	 * buf.append("\n- throughput        : " + throughput[i][j]); buf.append("\n- utilization       : " + utilization[i][j]);
	 * 
	 * } buf.append("\n\nAggregate values     : "); buf.append("\n- queue length       : " + scQueueLen[i]); buf.append("\n- residence time     : " +
	 * scResidTime[i]); buf.append("\n- throughput         : " + scThroughput[i]); buf.append("\n- utilization        : " + scUtilization[i]); } for
	 * (int j = 0; j < classes; j++) { buf.append("\n\nIndexes of class " + j); buf.append("\n- response time       : " + clsRespTime[j]);
	 * buf.append("\n- throughput          : " + clsRespTime[j]); } buf.append("\n\n"); buf.append("\nSystem Response Time    : " + sysResponseTime);
	 * buf.append("\nSystem Throughput       : " + sysThroughput); buf.append("\nSystem users            : " + sysNumJobs); return buf.toString(); }
	 */
