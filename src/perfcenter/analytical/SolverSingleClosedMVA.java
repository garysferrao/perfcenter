...
public class SolverSingleClosedMVA extends Solver {
	...
	/**
	 * array containing the throughput for each population and for each service center <br>
	 * [population][service center]
	 */
	protected double[][] interm_throughput;

	/**
	 * array containing the utilization for each population and for each service center <br>
	 * [population][service center]
	 */
	protected double[][] interm_utilization;

	/**
	 * array containing the queue length for each population and for each service center <br>
	 * [population][service center]
	 */
	protected double[][] interm_queueLen;

	/**
	 * array containing the residence time for each population and for each service center (residence time = time spent in queue + time spent in
	 * service) <br>
	 * [population][service center]
	 */
	...
	/**
	 * Initializes the solver with the system parameters. It must be called before trying to solve the model. <br>
	 * WARNING: This method changes the order of the stations (LI and DELAY before, LD after): the original order is restored only after calling the
	 * "solve" method (so if you solve the model, you don't need to reorder results, they are reordered by the "solve" method itself; otherwise, if
	 * you don't solve the model after calling "input" method, you may have some problems).
	 * 
	 * @param n
	 *            array of names of service centers.
	 * @param t
	 *            array of the types (LD or LI) of service centers.
	 * @param s
	 *            matrix of service times of the service centers.
	 * @param v
	 *            array of visits to the service centers.
	 * @return true if the operation is completed with success
	 */
	public boolean input(String[] n, int[] t, double[][] s, double[] v, int threadcount) {
		...
		if (!(countLI > 0 && countLI < t.length)) {
			// end NEW
			// TODO1: controllare la nuova condizione
			...
			// LI stations are put first (i.e. from position 0 to position
			// countLI-1)
			// LD stations are put after all LI stations (i.e. from position
			// count to the end)
	...
	// @author Stefano Omini
	/**
	 * A system is said to have sufficient capacity to process a given load <tt>lambda</tt> if no service center is saturated as a result of such a
	 * load. <br>
	...
	/*
	 * public void indexes() { //TODO1: non implementato!!!! } //overrides indexes() of SolverSingleClosedExact
	 */

	/**
	 * Solves a single class closed model using MVA algorithm.<br>
	 * "input(...)" method must have been called before solving the model!!<br>
	 * <br>
	 * Reference:<br>
	 * <em>
	 ...
	 * <br>
	 * 
	 * To solve a closed model with N customers, all the models with 0, 1, 2, ... N-1 customers must be solved before. This method allows to save the
	 * results for these intermediate models.
	 * 
	 * Reference:<br>
	 * <em>
	 *  G.Balbo, S.C.Bruell, L.Cerchio, D.Chiaberto, L.Molinatti<br>
	 ...
	 * 
	 * @param intermediate
	 *            true to save results for intermediate populations
	 */
	public void solve(boolean intermediate) {
		...
		if (loadIndep) {
			...
			// TODO1: aggiungere caso intermediate=true per LD (dopo averlo
			// implementato!!)
	...
	private void solveSingleLI() {
		...
		// calculation of throughput for the whole network and of queue length
		// for each station
		...
		for (n = 1; n <= customers; n++) {
			...
			for (k = 0; k < stations; k++) {
				...
				case Solver.DELAY:
					...
					break;
				case Solver.LI:
					// recursion uses the queue length of the model with n-1
					// customers
	...
	/** Solves a single class closed model with only LI stations, and multiserver station */
	private void solveSingleLIMultiServer() {
		int k;// index of station
		int n;// index of customer
		double sum;
		double pi[][] = new double[threads][customers];
		pi[0][0] = 1;
		// service demands: D = visits * servTime
		double[] D = new double[stations];
		...
		for (n = 1; n <= customers; n++) {
				...
				case Solver.LI:
					...
					if (threads == 1)
						...
						for (int p = 0; p < threads - 1; p++) {
							temp += (threads - p - 1) * pi[p][n - 1];
						}

						residenceTime[k] = D[k] * (1 + queueLen[k] + temp) / threads;
					}
			...
			if (n < customers) {
				...
				for (int j = 1; j < threads; j++) {
					pi[j][n] = totThroughput * pi[j - 1][n - 1] * servTime[0][0] / j;
					temp += (threads - j) * pi[j][n];
				}

				pi[0][n] = 1 - (totThroughput * servTime[0][0] + temp) / threads;
			}
		}// end loop through customers
	...
	/**
	 * Solves a single class closed model with only LI stations, using an approximated technique (which avoids recursion)
	 * 
	 * @author Stefano Omini
	 */
	// TODO1: non funziona ancora tanto bene, in particolare alcuni valori di
	// residence e queue sembrano poco precisi
	private void solveSingleLI_approx() {
		...
		boolean finished = false; // true, when the specified precision has
									// been reached
	...
	/**
	 * Solves a single class closed model with only LI stations. To solve a closed model with N customers, all the models with 0, 1, 2, ... N-1
	 * customers must be solved before. This method allows to save the results for these intermediate models.
	 * 
	 * @param intermediate
	 *            true to save results for intermediate populations
	 * 
	...
	private void solveSingleLI(boolean intermediate) {
		...
		for (n = 1; n <= customers; n++) {
			...
			for (k = 0; k < stations; k++) {
				...
				interm_totUser[n] += interm_queueLen[n][k]; // of course must be
															// equals to n
			}
			// end loop through stations
	...
	private void solveSingleLD() {
		...
		double[] Y = new double[customers + 1];// 1/throughput of composite
												// system
		 ...
		 * OLD
		 * 
		 * //loop over all stations, transform all stations in load dependent //it's not efficient must be changed for (stat = 0; stat < stations;
		 * stat++) { if (type[stat] != Solver.LD) { wcomp = servTime[stat][0]; if (type[stat] == Solver.DELAY) { for (cust = 1; cust <= customers;
		 * cust++) //TODO1: attenzione, qui va in out of bound, non c'� un elemento x ogni popolazione! servTime[stat][cust] = wcomp / cust; } else {
		 * for (cust = 1; cust <= customers; cust++) servTime[stat][cust] = servTime[stat][0]; } } }//end loop over all stations
		 */
	...
	private void solveSingleLD2() {
		...
		double[] Y = new double[customers + 1];// throughput of composite
												// system
		double ws;// waiting time
		double wcomp;// composite waiting time
		// double prod;//internal variable
		...
		for (m = noLI ? 1 : LICount; m < stations; m++) {
			...
			for (n = 1; n <= customers; n++) {
				...
				if (Math.abs(sum - 1) >= 0.01 && (n % 500 == 0 || n == customers))
					;
				// System.out.println(n+" ERROR Sum of prob : "+sum+" zeros
				// "+zeroCount);
			...
			// System.arraycopy(nsPrec[stations-1], 0, nsCorr[stations-1], 0,
			// customers +1);
			// System.arraycopy(usPrec[stations-1], 0, usCorr[stations-1], 0,
			// customers +1);
	...
	private void solveSingleLDErrCtr() {
		...
		/*
		 * //put all queue length to 0 for n = 0 customers in the network for (k = 0; k < LICount; k++) { queueLen[k] = 0; D[k] = servTime[k][0] *
		 * visits[k];//D = S * V to speed up }//end loop through stations
		 */
		// put all queue length to 0 for n = 0 customers in the network
		int counter = 0;
		for (k = 0; k < stations; k++) {
		...
		if (!noLI) {
			for (n = 1; n <= customers; n++) {
				...
				// 22-09-2003 DEK (Federico Granata)
				/*
				 * for (k = 0; k < LICount; k++) { switch (type[k]) { case Solver.DELAY: residenceTime[k] = D[k]; break; case Solver.LI:
				 * residenceTime[k] = D[k] * (1 + queueLen[k]); break; case Solver.LD://not supported use another method throw new
				 * IllegalArgumentException("error in solveSingleLI, type of service centers must be delay or load Independent"); }//end case
				 * 
				 * sum += residenceTime[k]; }//end loop through stations
				 */
				counter = 0;
				for (k = 0; k < stations; k++) {
					switch (type[k]) {
				...
				// DEBUG
				/*
				 * System.out.println("n = " + n); for (int i = 0; i < residenceTime.length; i++) { System.out.print(residenceTime[i]+ " "); }
				 * System.out.println(); for (int i = 0; i < queueLen.length; i++) { System.out.print(queueLen[i]+ " "); } System.out.println();
				 * System.out.println();
				 */
		...
		for (m = 1; m < stations; m++) {
			if (type[m] == Solver.LD) {
				...
				/*
				 * System.out.println("m = " + m); System.out.println("n = " + n); System.out.print("Y = "); for (int i = 0; i < Y.length; i++) {
				 * System.out.print(Y[i] + " "); } System.out.println(); System.out.println();
				 */

				for (n = 1; n <= customers; n++) {
					// calculation of waiting time at station m an of the
					// aggregated m-1
					// stations.
					ws = 0;
					wcomp = 0;
					...
					/*
					 * System.out.println("m = " + m); System.out.println("n = " + n); System.out.print("Y = "); for (int i = 0; i < Y.length; i++) {
					 * System.out.print(Y[i]+" "); } System.out.println(); System.out.println(); System.out.print("X = "); for (int i = 0; i <
					 * X.length; i++) { System.out.print(X[i]+" "); } System.out.println(); System.out.println(); for (int i = 0; i < nsCorr.length;
					 * i++) { for(int j = 0; j < nsCorr[i].length; j++) { System.out.print(nsCorr[i][j]+" "); } System.out.println(); }
					 * System.out.println(); for (int i = 0; i < usCorr.length; i++) { for(int j = 0; j < usCorr[i].length; j++) {
					 * System.out.print(usCorr[i][j]+" "); } System.out.println(); } System.out.println();
					 */
		...
		/*
		 * System.out.println("m = " + m); System.out.println("n = " + n); System.out.print("Y = "); for (int i = 0; i < Y.length; i++) {
		 * System.out.print(Y[i]+" "); } System.out.println(); System.out.println(); System.out.print("X =
		 * "); for (int i = 0; i < X.length; i++) { System.out.print(X[i]+" "); } System.out.println(); System.out.println(); for (int i = 0; i <
		 * nsPrec.length; i++) { for(int j = 0; j < nsPrec[i].length; j++) { System.out.print(nsPrec[i][j]+" "); } System.out.println(); }
		 * System.out.println(); for (int i = 0; i < usPrec.length; i++) { for(int j = 0; j < usPrec[i].length; j++) {
		 * System.out.print(usPrec[i][j]+" "); } System.out.println(); } System.out.println();
		 */
	...
	private void solveSingleLDErrCtrFast() {
		...
		// put queue length = 0 for all LI stations, for n = 0 customers in the
		// network
		for (k = 0; k < LICount; k++) {
			queueLen[k] = 0.0;
			...
			for (n = 1; n <= customers; n++) {
				...
				for (k = 0; k < LICount; k++) {
					...
					case Solver.DELAY:
						...
						break;
					case Solver.LI:
						// recursion: uses queue length of the model with n-1
						// customers
						residenceTime[k] = D[k] * (1 + queueLen[k]);
						break;
					case Solver.LD:
		...
		// OLD
		// for (m = noLI ? 1 : LICount; m < stations; m++) {
		// TODO1: se noLI � vero non dovrebbe partire da 0??

		// TODO1: a meno che:: (COMMENTO tratto da solveSingleLD)
		// loop over all stations except the first, it merges a station at every
		// pass when all the stations are merged, the system is solved
		...
		for (m = m0; m < stations; m++) {
			...
			for (n = 1; n <= customers; n++) {
				// calculation of waiting time at station m an of the aggregated
				// m-1
				// stations.
				ws = 0.0;
				wcomp = 0.0;
				...
				// expMultCorr[0])) - 1) > 0.0000000000001)
				// System.out.println(n + " ERROR Sum of prob : " +
				// (usPrec[m][n] + (pCorr[0] * Math.pow(MIN,
				// -expMultCorr[0]))));
				...
				/*
				 * System.out.println("m = " + m); System.out.println("n = " + n); System.out.print("Y = "); for (int i = 0; i < Y.length; i++) {
				 * System.out.print(Y[i] + " "); } System.out.println(); System.out.println(); System.out.print("X = "); for (int i = 0; i < X.length;
				 * i++) { System.out.print(X[i] + " "); } System.out.println(); System.out.println(); for (int i = 0; i < nsPrec.length; i++) { for
				 * (int j = 0; j < nsPrec[i].length; j++) { System.out.print(nsPrec[i][j] + " "); } System.out.println(); } System.out.println(); for
				 * (int i = 0; i < usPrec.length; i++) { for (int j = 0; j < usPrec[i].length; j++) { System.out.print(usPrec[i][j] + " "); }
		...
		// calculation of other parameters
		// TODO1: perch� moltiplica per visits[stations-1] ??
		totThroughput = X[customers] / visits[stations - 1];
		for (m = noLI ? 0 : LICount; m < stations; m++) {
			throughput[m] = totThroughput * visits[m];
			if (type[m] == Solver.DELAY)
	 ...
	 * Solves a model with LI and LD stations
	 */
	// TODO1: by DEK (Federico Granata) ti sei impestato nel metodo meno testato,
	// e piu' incasinato! complimenti
	// e' il meno testato perche' Serazzi non mi e' mai stato convito ad usare
	// un metodo la cui stabilita'
	// non era nota e nemmeno un bound sull'errore..
	// inoltre non e' vero che sia piu' veloce lo e' solo se le probabilita'
	// significative sono poche.
	// e' poco testato perche' non ho avuto granche tempo di lavorarci sopra
	// visto che non ero autorizzato.
	// e' inoltre poco ottimizzato (rispetto a quello che si puo' fare in quanto
	// c'era poco tempo)
	// saprei spiegarti molto meglio il metoto Fast, sicuramente stabile.
	// comunque l'idea che sta dietro a questo deriva da quello sopra, in questo
	// caso dico approx perche'
	// non tutti gli elemnti vengono sommati ad ogni passo. il motivo e'
	// semplice non servono,
	// sommare elemnti molto molto piccoli ad uno grande e' inutile. questo
	// metodo fa esattamente questo.
	private void solveSingleLDErrCtrFaster() {
		...
		// TODO1: questo dovrebbero essere il throughput: quale differenza c'�
		// tra X e Y??
		// R: non hai guardato la bibliografia? l'articolo di balbo bruell????
		// si X e Y sono il throughput ma calcolato in maniera diversa, mi
		// spiego X e' quello
		// classico da Lazowska, Y invece vience calcolato mano a mano ponendo 1
		// alla volta il tempo di
		// servizio del nodo pari a 0
		double[] X = new double[customers + 1];
		double MIN = 1.0E-290;
		double MAX = 1.0E290;

		// TODO1: che cosa sono puntProb e expMult?
		// R1: expMult e' il esponente che serve come nei metodi che vedi sopra
		// per tenere
		// conto delle grandi oscillazioni delle probabilita' in pratica devo
		// avere un array grande
		// N+1 customers che contine il vaolre dell-esponente del moltiplicatore
		// di ogni probabilita'
		// anche questo problema e' nella mia tesi... quello che succede e' che
		// nei sistemi LD le probabilita'
		// mentre si risolve il sistema oscillano tra valori molto piccoli
		// (dipende dai parametri dei sistema
		// ma possono arrivare a 10^-1000 )e vaolri significativi (10^/2)
		// ovvimanete un semplice double non farebbe
		// altro che trasformare il numero in 0 e quindi perdere l'informazione
		// connessa a quella probabilita'.
		// il risultato e' un errore anche molto consistente nella soluzione.
		//
		// R2:puntProb, e' un "puntatore" cioe' punta al valore
		// puntProb[0] contiene il numero di elemnti usati di puntProb, per
		// evitare di farsi tutto il giro inutile dell'array
		// ad ogni passo. tiene traccia degli elemnti significativi nel sistema.
		// se uan probabilita' e'
		...
		// TODO1: queste dovrebbero essere probabilit� marginali di avere j
		// customer nalla stazione (con o<j<n)
		double[] pPrec = new double[customers + 1];
		double[] pCorr = new double[customers + 1];
		...
		// recoursively starting from 0 customer in network to max customers
		// later all the LI subnetworks will be transformed into a single LD
		// station: service time for n customers
		...
		// loop over all LD stations (except the first which does not require to
		// be
		...
		for (m = noLI ? 1 : LICount; m < stations; m++) {
			...
			for (n = 1; n <= customers; n++) {
				// calculation of waiting time of station m and of the
				// aggregated m-1
				// stations.
				ws = 0.0;
				wcomp = 0.0;
				...
				for (k = 1; k < n; k++) {
					pCorr[k] = servTime[m][k] * X[n] * pPrec[k - 1];
					// TODO1: exp ?? Forse serve per probabilit� troppo basse??
					...
					if (pCorr[k] < MIN) {
						...
						// TODO1: idem per prob alte??
						// R: esatto pero' cosidera che alte vuole dire tenendo
						// conto del moltiplicatore...
					} else if (pCorr[k] > MAX) {
						pCorr[k] *= MIN;
						expMultCorr[k]--;
					}
					// TODO1: punto non chiaro: cosa succede??
					// R: quando il moltiplicatore per la prob k-esima e' 0
					// aumento il numero
					if (expMultCorr[k] == 0) {
						puntProb[0]++;
		...
		for (m = 0; m < stations; m++) {
			...
			totUser += queueLen[m]; // of course should be equal to the number
									// of customers
		}
	...
	/**
	 * Solves a model with LI and LD stations. To solve a closed model with N customers, all the models with 0, 1, 2, ... N-1 customers must be solved
	 * before. This method allows to save the results for these intermediate models.
	 * 
	 * @param intermediate
	 *            true to save results for intermediate populations
	 * 
	...
	private void solveSingleLDErrCtrFaster(boolean intermediate) {
		// TODO1: ancora da sistemare!!! Non si hanno ancora i risultati
		// intermedi...
		if (!intermediate) {
			// intermediate results mustn't be saved, then use
			// the base method
		...
		for (m = noLI ? 1 : LICount; m < stations; m++) {
			...
			for (n = 1; n <= customers; n++) {
				// calculation of waiting time at station m and of the
				// aggregated m-1
				// stations.
				ws = 0.0;
				wcomp = 0.0;
		...
		/*
		 * //calculation of other parameters totThroughput = interm_totThroughput[customers] = X[customers] / visits[stations - 1]; for (m = noLI ? 0
		 * : LICount; m < stations; m++) { throughput[m] = totThroughput * visits[m]; if (type[m] == Solver.DELAY) utilization[m] = visits[m] *
		 * servTime[m][0]; else utilization[m] = usPrec[m][customers]; queueLen[m] = nsPrec[m][customers]; residenceTime[m] = queueLen[m] /
		 * totThroughput; }
		 */
